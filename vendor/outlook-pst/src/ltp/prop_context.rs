...
const MAX_PROPERTY_BYTES: u64 = 50 * 1024 * 1024;
const MAX_MULTI_VALUE_ITEMS: usize = 1_000_000;
    ...
    fn read(f: &mut dyn Read, prop_type: PropertyType) -> io::Result<Self> {
        match prop_type {
            ...
            PropertyType::String8 => {
                ...
                f.take(MAX_PROPERTY_BYTES + 1).read_to_end(&mut buffer)?;
                if buffer.len() > MAX_PROPERTY_BYTES as usize {
                    buffer.truncate(MAX_PROPERTY_BYTES as usize);
                }
            ...
            PropertyType::Unicode => {
                let mut buffer = Vec::new();
                let mut limited = f.take(MAX_PROPERTY_BYTES + 1);
                while let Ok(ch) = limited.read_u16::<LittleEndian>() {
                    if ch == 0 {
                        break;
                    }
                    buffer.push(ch);
            ...
            PropertyType::MultipleInteger16 => {
                ...
                let mut limited = f.take(MAX_PROPERTY_BYTES + 1);
                while let Ok(value) = limited.read_i16::<LittleEndian>() {
                    values.push(value);
                }
            ...
            PropertyType::MultipleInteger32 => {
                let mut values = Vec::new();
                let mut limited = f.take(MAX_PROPERTY_BYTES + 1);
                while let Ok(value) = limited.read_i32::<LittleEndian>() {
                    values.push(value);
                }
            ...
            PropertyType::MultipleFloating32 => {
                let mut values = Vec::new();
                let mut limited = f.take(MAX_PROPERTY_BYTES + 1);
                while let Ok(value) = limited.read_f32::<LittleEndian>() {
                    values.push(value);
                }
            ...
            PropertyType::MultipleFloating64 => {
                let mut values = Vec::new();
                let mut limited = f.take(MAX_PROPERTY_BYTES + 1);
                while let Ok(value) = limited.read_f64::<LittleEndian>() {
                    values.push(value);
                }
            ...
            PropertyType::MultipleCurrency => {
                let mut values = Vec::new();
                let mut limited = f.take(MAX_PROPERTY_BYTES + 1);
                while let Ok(value) = limited.read_i64::<LittleEndian>() {
                    values.push(value);
                }
            ...
            PropertyType::MultipleString8 => {
                ...
                if count > MAX_MULTI_VALUE_ITEMS {
                    return Err(io::Error::new(
                        io::ErrorKind::InvalidData,
                        "multiple string8 property too large",
                    ));
                }
                ...
                for i in 0..offsets.len() {
                    ...
                    let mut buffer = if i < offsets.len() - 1 {
                        ...
                        if next - start > MAX_PROPERTY_BYTES as usize {
                            return Err(io::Error::new(
                                io::ErrorKind::InvalidData,
                                "multiple string8 property too large",
                            ));
            ...
            PropertyType::MultipleUnicode => {
                ...
                if count > MAX_MULTI_VALUE_ITEMS {
                    return Err(io::Error::new(
                        io::ErrorKind::InvalidData,
                        "multiple unicode property too large",
                    ));
                }
                ...
                for i in 0..offsets.len() {
                    ...
                    if i < offsets.len() - 1 {
                        ...
                        while start < next {
                            if buffer.len() * mem::size_of::<u16>() >= MAX_PROPERTY_BYTES as usize {
                                break;
                            }
            ...
            PropertyType::MultipleGuid => {
                ...
                if count > MAX_MULTI_VALUE_ITEMS {
                    return Err(io::Error::new(
                        io::ErrorKind::InvalidData,
                        "multiple guid property too large",
                    ));
                }
            ...
            PropertyType::MultipleBinary => {
                ...
                if count > MAX_MULTI_VALUE_ITEMS {
                    return Err(io::Error::new(
                        io::ErrorKind::InvalidData,
                        "multiple binary property too large",
                    ));
                }
